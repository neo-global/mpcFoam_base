...
{
    Pout<<"zoneDistribute::updateStencil"<<endl;
...
{
    Pout<<"zoneDistribute::setUpCommforZone"<<endl;
    ...
    if (Pstream::parRun())
    {
    	...
    	Pout<<"Parallel run detected."<<endl;
    	...
    	Pout<<"Loop completed 1."<<endl;

        PstreamBuffers pBufs(Pstream::commsTypes::nonBlocking);

        // Stream data into buffer
    	Pout<<"stream data into buffer.."<<endl;
    	...
    	Pout<<"get data from send buffer.."<<endl;
        for (label domain = 0; domain < Pstream::nProcs(); domain++)
        {
            send_[domain].clear();

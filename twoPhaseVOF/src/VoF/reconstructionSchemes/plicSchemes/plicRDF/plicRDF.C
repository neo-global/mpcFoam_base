...
{
    Pout<<"plicRDF. Interpolate normal"<<endl;
...
{
    Pout<<"plicRDF::gradSurf"<<endl;
    leastSquareGrad<scalar> lsGrad("polyDegree1", mesh_.geometricD());
    zoneDistribute& exchangeFields = zoneDistribute::New(mesh_);
   
    Pout<<"exchange fields...setUpCommforZone. calling zoneDistribute."<<endl;
    exchangeFields.setUpCommforZone(interfaceCell_, false);
    ...
    forAll(interfaceLabels_, i)
    {
        Pout<<"for All interface Labels"<<endl;
        ...
        {
            Pout<<"get cellCentre values and append "<<endl;
            cellCentre.append
            (
                exchangeFields.getValue(mesh_.C(), mapCC, gblIdx)
            );
            Pout<<"get phiValues and append "<<endl;
            phiValues.append
            (
 	...
 	Pout<<"update cellCentre"<<endl;
        cellCentre -= mesh_.C()[celli];
 	Pout<<"update lsGrad.grad()"<<endl;
        interfaceNormal_[i] = lsGrad.grad(cellCentre, phiValues);
    }
    Pout<<"plicRDF::gradSurf complete."<<endl;
}


void Foam::reconstruction::plicRDF::setInitNormals(bool interpolate)
{
    Pout<<"plicRDF. setInitNormals..."<<endl;
...
{
    Pout<<"plicRDF.calcResidual"<<endl;
    zoneDistribute& exchangeFields = zoneDistribute::New(mesh_);
    exchangeFields.setUpCommforZone(interfaceCell_,false);
...
{
    Pout<<"plicRDF. centreAndNormalBC"<<endl;
